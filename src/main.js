const scenarios = {
  rbac: {
    ...
    nodes: [["workspaces.ts", "entry", "Workspace routes", 17, 46], ["authorize.ts", "change", "Policy guard", 40, 23], ["membership.ts", "change", "Role lookup", 40, 70], ["role.ts", "change", "Permission matrix", 63, 23], ["projects.ts", "verify", "Project routes", 63, 70], ["workspace.spec.ts", "verify", "Access tests", 84, 46]],
  ...
  audit: {
    ...
    nodes: [["workspaces.ts", "entry", "Workspace routes", 17, 46], ["audit.ts", "change", "Audit writer", 40, 22], ["publisher.ts", "change", "Event bus", 40, 71], ["audit.ts", "change", "Audit worker", 63, 22], ["audit-event.ts", "change", "Event schema", 63, 71], ["audit.spec.ts", "verify", "Audit tests", 84, 46]],
  ...
  billing: {
    ...
    nodes: [["webhooks.ts", "entry", "Webhook intake", 17, 46], ["publisher.ts", "change", "Durable queue", 40, 22], ["webhook.ts", "change", "Delivery state", 40, 71], ["billing.ts", "change", "Billing worker", 63, 22], ["subscription.ts", "change", "Subscription write", 63, 71], ["billing.spec.ts", "verify", "Worker tests", 84, 46]], edges: [[0, 1], [0, 2], [1, 3], [2, 3], [2, 4], [3, 5], [4, 5]], risk: ["High risk", "Webhooks are retried by providers. The worker must persist delivery state before changing a subscription and tolerate duplicate events."], context: ["31%", "14 files selected from 186. Payment UI is intentionally outside the request path."],
